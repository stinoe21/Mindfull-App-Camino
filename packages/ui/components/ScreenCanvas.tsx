...
// Een kind met flex 1 (een lege View als opvulling) laat de wrapper meegroeien,
// zodat een scherm zijn knoppen onderaan kan zetten (check-in, 13 september 2026).
function rekt(kind: ReactNode): boolean {
  const stijl = isValidElement<{ style?: unknown }>(kind) ? kind.props.style : undefined;
  return typeof stijl === "object" && stijl !== null && !Array.isArray(stijl) && (stijl as { flex?: number }).flex === 1;
}
...
function gestaffeld(children: ReactNode, gap: number, centreer: boolean): ReactNode {
  return Children.map(children, (kind, i) =>
    kind === null || kind === undefined || kind === false ? kind : (
      <Verschijn
        vertraging={i * STAGGER}
        gapOuder={gap}
        style={{ alignSelf: "stretch", alignItems: centreer ? "center" : "stretch", flex: rekt(kind) ? 1 : undefined }}
      >
        {kind}
      </Verschijn>
    )
