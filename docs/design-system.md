...
| **Contrastcontrole** | Gemeten op 18 september 2026 met `node scripts/contrast.mjs`, tegen WCAG 2.1 AA (4,5 op 1 voor tekst, 3 op 1 voor wat geen tekst is). Van de 42 combinaties van tekst en vlak die de app gebruikt, halen er 41 de eis; de laagste daarvan is secundaire tekst op de Hulplijnkaart met 5,58. **Eén zakt: de overline van 11 punten en het pijltje in merkblauw op de regentint van de weerkaart op Home (`primary200`), 4,01 op 1.** Voorstel: op de regenkaart de gewone tekstkleur voor de overline (9,99), of de regentint een stap lichter. Dat is een besluit over tokens en dus van Stijn. De metingen staan in `packages/ui/tokens/contrast.ts` en `npm test` bewaakt ze: een nieuwe combinatie die zakt, breekt de test. |
