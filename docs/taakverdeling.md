...
**3. Weer-check-in en landelijk weerbericht.** Aan de achterkant het verste onderdeel: `submit_weather()` en `weather_today()` staan in main, inclusief het slot per dagdeel. De aanroepen staan in `backend-draaiboek.md`. Je blokkeert op de vraagteksten van de vier sliders en op welke combinatie tot welk van de vijf weerbeelden leidt (open punt in `datamodel.md`); de schermen en states eromheen kunnen wel vast. Drie ankers: het combineren van sliders naar weerbeeld gebeurt **op het toestel** en de sliderwaarden gaan nooit naar de server; de foutmelding "dit dagdeel al bijgedragen" is een normale flow en geen bug, de check-in werkt dan alleen het eigen weer bij; en inchecken kan altijd, de app leest de profielrij niet (sinds 15 september 2026).
