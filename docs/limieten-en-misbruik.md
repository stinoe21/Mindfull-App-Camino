...
- **Inloggen gaat sinds 28 augustus met e-mail en wachtwoord** (`signInWithPassword` en `signUp` in `apps/mobile/src/app/(onboarding)/inloggen.tsx`, PR #54), en zonder account kom je de app niet in. Er is geen e-mailcode meer en geen anonieme login (`enable_anonymous_sign_ins = false`). Sinds 17 september 2026 eist de app minimaal 8 tekens voor een nieuw wachtwoord (`MIN_WACHTWOORD` in `features/auth/accountHerstel.ts`; wie inlogt met een ouder, korter wachtwoord komt er gewoon in) en controleert hij de vorm van het e-mailadres. Zet het minimum in het dashboard ook op 8, zie de checklist hieronder. **Wachtwoord vergeten en de bevestigingsmail** lopen sinds dezelfde dag via een maillink die de app opent op `wachtwoord-nieuw` en `mail-bevestigd`; de code in de link wordt met PKCE ingewisseld, dus alleen op het toestel dat de mail aanvroeg. Het antwoord op het herstelscherm is altijd hetzelfde, of het adres nu bestaat of niet. Beide mails tellen mee voor de SMTP-limiet uit sectie 1. De knoppen voor Apple en Google staan er wel, maar doen nog niets.
